<template>
	<div id="container">
		...
		<div class="remain-time" @click="onClickRemainTime">{{ remainSecond.toFixed(2) }}<span>s</span></div>
		...
		<div id="setting-panel" :class="{ in: isShowSettings, shake: isShaking }">
			...
			<div class="form-group">
				...
				<input ref="settingSecondInput" type="number" id="capacity" v-model="settingSecond" tabindex="-1" @keydown.prevent.stop.enter="hideSettings">
				<span>s</span>
			</div>
			<div class="form-group">
...
<script>
	export default {
		...
		methods: {
			...
			onClickRemainTime() {
				this.showSettings();
				this.$refs.settingSecondInput.focus();
			},
			showSettings() {
				this.settingSecond = this.capacitySecond;
